    ...
    <div>
        ...
        <hr />
        {
            blogs.length > 0 ? (<div className='space-y-4 mt-5'>
                {
                    blogs.map((blog) => (
                        <Link to={`/blogs/${blog?._id}`} className='flex flex-col sm:flex-row sm:items-center gap-4 shadow-sm px-8 py-4' key={blog._id} >
            ...
            </div>):(<div className='p-8'>No Related Blogs</div>)
        }
    </div>
  )
}
